...
controller.inserusu = async(req, res, next) => {
    const d = req.body.dc;
    const u = req.body.us;
    const c = req.body.cl;
    const r = req.body.rl;
    ...
    cnn.query('INSERT INTO usuarios SET?', { doccli: d, nomusu: u, clave: c, rol: r, estado: e, imagen: i }, (err, resbd) => {
...
controller.consig = async(req, res, next) => {
    cnn.query('SELECT * FROM cuentas', (err, resbd) => {
        if (err) {
            next(new Error(err))
            console.log("Error en la consulta")
